...
#比較as.POSIXct() 輸出亳秒所以可以運算 和 as.POSIXlt  輸出日期格式列表 [1]sec [2]min [3]hour  [4]mday 
...
#查詢applenews的欄位   "content"  "title"    "dt"       "category" "clicked" 
names(applenews)
...
#將欄位title到clicked的資料都選出來  因為內容太多所以使用head()秀出來
select(applenews,title:clicked) %>% head()
...
#apllenews_category分類有幾類
n_distinct(applenews$category)
...
#scale Function可以調整圖表的刻度，大小，顏色，顯示的排序等....
#scale_x_discrete操縱分類尺度
